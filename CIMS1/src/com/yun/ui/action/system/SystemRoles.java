...
public class SystemRoles
{
	 ...
	 */
	private static final long serialVersionUID = 2012121215185729698L;
    ...
    */
	...
	private String getpageListUrl()
	{
        ...
        String pageListOrderByItem = com.colorwinglib.Requests.GetString(request, "OrderByItem", "DatabaseRolesID");//排序排序的字段名
    ...
    // 表主键字段
		int Databaserolesid =  com.colorwinglib.Requests.GetInt(request, "databaserolesid",0);//DatabaseRolesID  数据类型:int
    ...
    // 表主键字段
		...
		com.yun.entities.culture.SystemRolesInfo objSystemRolesInfo = com.yun.bll.culture.SystemRolesProvider.GetSystemRolesInfoByDatabaserolesid(Databaserolesid);//实体类
        if (objSystemRolesInfo != null)
        {	
			request.setAttribute("Databaserolesid", objSystemRolesInfo.getDatabaserolesid());//DatabaseRolesID  数据类型:int
    ...
    protected void insert()
    {
        ...
        if (!ServerValidate())
        {
			//进行赋值验证失败时用
			DrpInit();
			request.setAttribute("Databaserolesid", Databaserolesid);//DatabaseRolesID  数据类型:int
    ...
    // 表主键字段
		...
		if(SystemAdminProvider.GetListByRolesid(Databaserolesid).size()>0)
		{
 			String js = com.colorwinglib.Javascript.AlertAndRedirect(this.request, this.response, "角色中存在用户不能删除", "?" + pageListUrl);
 			request.setAttribute("pageJavaScripStar", js);
 			return;
		}

		boolean isRun = com.yun.bll.culture.SystemRolesProvider.DeleteByDatabaserolesid(Databaserolesid);//直接删除
