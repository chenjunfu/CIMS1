...
function preloadIcons()
{
	...
	icons[4] = new Image();
	icons[4].src = "image/tree/img/dept.gif";
	icons[5] = new Image();
	icons[5].src = "image/tree/img/dept.gif";
}
// Create the tree
	...
	if (nodes.length > 0)
	{
		...
		if (startNode != 0)
		{
			...
			if (nodesValue != null)
			{
				...
				if(nodeValues[3].indexOf("javascript:")!=-1)
				document.write("<a onclick=\"" + nodeValues[3] + "\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\"><img src=\"image/tree/img/dept.gif\" align=\"absbottom\" alt=\"\" />" + nodeValues[2] + "</a><br />");
				else
				document.write("<a href=\"" + nodeValues[3] + "\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\"><img src=\"image/tree/img/dept.gif\" align=\"absbottom\" alt=\"\" />" + nodeValues[2] + "</a><br />");
			}
		}
		else
		{
			document.write("<img src=\"image/tree/img/base.gif\" align=\"absbottom\" alt=\"\" /><br />");
		}
	...
	{ 
		if (nodes[i] != null)
		{
			...
			if (nodeValues[1] == parentNode)
			{
				...
				// Start link
				if(nodeValues[3].indexOf("javascript:")!=-1)
				document.write("<a onclick=\"" + nodeValues[3] + "\" target=\"mainFrame\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\">");
				else
				document.write("<a href=\"SystemPersonnel.do?pagetype=tree&target=personnel\" target=\"I1\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\">");
				...
				if (hcn)//是否有子节点
				{
					document.write("<img id=\"icon" + nodeValues[0] + "\" src=\"image/tree/img/dept")
					if (ino)//是否打开
						document.write("");
				...
				else
				{
					...
					else
					{
						...
						//document.write(".gif\" align=\"absbottom\" alt=\"Folder\" />");
						document.write("<img id=\"icon" + nodeValues[0] + "\" src=\"image/tree/img/personnel.png\" style='width:18px;height:18px' align=\"absbottom\" alt=\"Page\" />");
					}
				}
