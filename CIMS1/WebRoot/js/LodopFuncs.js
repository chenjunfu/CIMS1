...
function PrintSet(title,tabInDiv)
{			
	LODOP.PRINT_INIT(title);
	var style="  <style> table,td,th {border-width: 1px;border-style: solid;border-collapse: collapse;border-color: black}table{width:100%;}  table thead tr{font-size: 12px;font-weight: bold;color: black;background-color: white;font-size: 15px;}</style>";
	var PrintArea=$('#'+tabInDiv+'').children('table').first().html();
	PrintArea="<table style='width:100% table-layout: fixed;word-break:break-all;word-wrap:normal;' >"+PrintArea+"</table>"
 	LODOP.ADD_PRINT_TABLE("5%","1%","98%","88%",style+PrintArea);
	LODOP.SET_PRINT_STYLEA(0,"TableHeightScope",0);	
	LODOP.ADD_PRINT_TEXT(3,653,135,20,"总页号：第#页/共&页");
	LODOP.SET_PRINT_STYLEA(0,"ItemType",2);
	LODOP.SET_PRINT_STYLEA(0,"Horient",1);	
	LODOP.ADD_PRINT_TEXT(3,34,196,20,title);
