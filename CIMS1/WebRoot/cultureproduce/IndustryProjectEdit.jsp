...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="tbEdit"  >
  <tr>
    ...
    <td width="20%">
    <input type="hidden" name="databaseprojectid" value="${Databaseprojectid }" id="databaseprojectid"/>
      <input name="projectid" type="text" id="txtProjectidEdit" value="${Projectid}" title="项目编号" onblur="validateno()" size="20" maxlength="20" />
     <div id="validateno" style="color: red;display: none;"></div>
      </td>
    <td width="13%" >项目名称 </td>
    <td width="20%" colspan="2"><input name="projectname" type="text" id="txtProjectnameEdit" value="${Projectname}" onblur="validatename()" title="项目名称" size="20" maxlength="50" />
...
<script type="text/javascript">document.getElementById('dropTdpzEdit').value='${Tdpz}';</script>
  ...
  <tr>
	...
	<tr >
    <td  style="border: 1px solid #cedff7;" colspan="8">
     <input type="button" value="上传图片" onclick="$('#upload').dialog('open')" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="ListShow();">获取新图片</a>
    </td>
    	</tr>
  <tr >
  
    <td colspan="8" style="border: 0px;margin: 0px;padding: 0px;">
  ...
  <c:if test="${param.pagetype=='gotoinsert'}">
<s:label value="暂无图片"></s:label>
</c:if>  
  <c:if test="${param.pagetype!='gotoinsert'}">
    请稍等，正在加载.....
</c:if>  
  ...
  <tr>
    ...
    <div align="center">
      ...
      <input type="button" name="btnReturn" id="btnReturn" value="" onclick="window.location.href='${pageListUrl}&pagetype=updata'" />
      </div>
      </td>
  </tr>
</table>
...
<div  id="upload" style="display: none;"  title="上传图片">
...
<table border="0" cellpadding="0" cellspacing="0" id="tbEditk" style="border: 0px solid; margin: 0px;padding: 0px">
  <tr>
    ...
    <td width="603"  align="left" style="border-left: 1px solid #cedff7;" >&nbsp;
...
<div id="delpic" style="position: absolute;width: 100px;height: auto;padding-left:20px;border: 0px solid red;color: red;display: none" >
单击删除图片
</div>
...
{	
    ...
    var delWidth=$("#delpic").width();
    var delLeft=parseInt((basewidth/2+baseleft)-(delWidth/2));
    var delTop=basetop+baseHeight;
	...
	$("#delpic").css({left:delLeft+"px",top:delTop+"px"})
	$("#delpic").stop(true,true).slideDown();
}
function closepic(){
$("#picmax").css({display:"none"})
$("#delpic").stop(true,true).hide();
}

function delpic(picid)
{	
	var url="<%=basePath%>cultureproduce/Projectpic.do?pagetype=delete&projectpicid="+picid;
	if(confirm("确定删除图片"))
	{
...
function validateno()
{
 ...
 {	
 	var Databaseprojectid=$("#databaseprojectid").val();
 	...
 	if(Databaseprojectid&&Databaseprojectid=="")
 	{
 		Databaseprojectid=0;
 	}
